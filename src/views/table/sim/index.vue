  ...
  <el-container>
      ...
      <el-row :gutter="10">
        <el-col :span="4">
          ...
          <el-select v-model="departmentId" placeholder="请选择部门" size="mini" clearable>
            <el-option v-for="item in bumens" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-col>
        <el-col :span="4">
          <span>运营商：</span>
        ...
        <el-col :span="4">
          ...
          <el-select v-model="expiredId" placeholder="请选择质保情况" size="mini" clearable>
            <el-option v-for="item in expireds" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-col>
        <el-col :span="4">
    ...
    <el-main>
      <el-table :data="tableData" highlight-current-row header-row-class-name="rowtitle">
        ...
        <el-table-column property="departmentName" label="公司" />
        <el-table-column property="cityName" label="属地化" />
        <el-table-column property="providerName" label="运营商" />
        <el-table-column property="num" label="电话号码" />
        <el-table-column property="orderNo" label="序号" />
    ...
    <el-dialog title="添加SIM卡" :visible.sync="tjBox" width="30%">
      <el-form :model="tjform">
        <el-form-item label="电话号码：" :label-width="formWidth" required>
          <el-input v-model="tjform.num" placeholder="请输入电话号码" />
        </el-form-item>
        <el-form-item label="属地城市：" :label-width="formWidth" required>
          <el-input v-model="tjform.cityName" placeholder="请输入属地城市" />
        </el-form-item>
        <el-form-item label="运营商：" :label-width="formWidth" required>
        ...
        <el-form-item label="所属部门：" :label-width="formWidth" required>
          <el-select v-model="tjform.departmentId" placeholder="请选择所属部门" clearable>
            <el-option v-for="item in bumens" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="流量到期时间：" :label-width="formWidth" required>
          <el-date-picker v-model="tjform.expiredAt" type="date" placeholder="请选择流量到期时间" />
        </el-form-item>
        <el-form-item label="序号：" :label-width="formWidth" required>
          <el-input v-model="tjform.orderNo" placeholder="请输入数字序号" @change="valNum" />
        </el-form-item>
        <el-form-item label="使用情况：" :label-width="formWidth" required  >
    ...
    <el-dialog title="修改SIM卡信息" :visible.sync="xgBox" width="30%">
      <el-form :model="xgform">
        <el-form-item label="电话号码：" :label-width="formWidth" required>
          <el-input v-model="xgform.num" placeholder="请输入电话号码" />
        </el-form-item>
        <el-form-item label="属地城市：" :label-width="formWidth" required>
          <el-input v-model="xgform.cityName" placeholder="请输入属地城市" />
        </el-form-item>
        <el-form-item label="运营商：" :label-width="formWidth" required>
        ...
        <el-form-item label="流量到期时间：" :label-width="formWidth" required>
          <el-date-picker v-model="xgform.expiredAt" type="date" placeholder="请选择流量到期时间" />
        </el-form-item>
        <el-form-item label="序号：" :label-width="formWidth" required>
          <el-input v-model="xgform.orderNo" placeholder="请输入数字序号" @change="valNum" />
        </el-form-item>
        <el-form-item label="使用情况：" :label-width="formWidth" required>
      ...
      <div slot="footer">
        <el-button size="mini" @click="xgfalse">取 消</el-button>
        <el-button type="primary" size="mini" @click="xgTrue">确 定</el-button>
      </div>
    </el-dialog>
  </el-container>
</template>
...
export default {
  ...
  methods: {
    ...
    tjTrue() {
      switch ("") {
        case this.tjform.num:
          this.$message.error("请输入电话号码");
          break;
        case this.tjform.cityName:
          this.$message.error("请输入属地城市");
          break;
        case this.tjform.provider:
          this.$message.error("请选择运营商");
          break;
        case this.tjform.departmentId:
          this.$message.error("请选择所属部门");
          break;
        case this.tjform.expiredAt:
          this.$message.error("请选择流量到期时间");
          break;
        case this.tjform.orderNo:
          this.$message.error("请输入排序号");
          break;
        case this.tjform.status:
          this.$message.error("请选择SIM状态");
          break;
        default:
          Axios({
            method: "post",
