...
<el-container>
  ...
  <el-main>
    <el-table :data="tableData" :highlight-current-row="true">
      ...
      <el-table-column label="网络类型" property="simProvider">
        <template slot-scope="simProvider">
          <span v-if="simProvider.row.simProvider ==='未绑定 SIM 卡'" style="color:#d0d0d0;">{{simProvider.row.simProvider}}</span>
      ...
      <el-table-column label="安装日期">
        <template slot-scope="date">
          <span v-if="date.row.deviceInstalledAt === '未安装'" style="color:#d0d0d0">{{date.row.deviceInstalledAt}}</span>
    ...
    <!-- 分页 -->
    <el-pagination background layout="prev, pager, next" :total="total" :page-size="8" @current-change="pageChange" style="float:right;margin-top:5px;"></el-pagination>
  </el-main>
  <!-- 修改框 -->
  <el-dialog title="新增设备" :visible.sync="dialogFormVisible" width="60%">
    <el-form :model="form">
      <el-form-item label="设备编码" :label-width="formLabelWidth">
        ...
        <label class="miniTitle">安裝日期</label>
        <el-date-picker v-model="form.installedAt" type="date" placeholder="请选择安装日期" @change="ATdate"></el-date-picker>
      </el-form-item>
      <el-form-item label="设备类型" :label-width="formLabelWidth">
        <el-select v-model="form.type" placeholder="请选择设备类型">
          <el-option v-for="item in formtypes" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">设备厂商</label>
        <el-select v-model="form.manufactureId" placeholder="请选择设备厂商">
          <el-option v-for="item in changjias" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">设备分类</label>
        <el-select v-model="form.deviceCategory" placeholder="请选择设备分类">
          <el-option v-for="item in deviceCategoryIds" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
    ...
    <el-form :model="form">
      <el-form-item label="电压等级" :label-width="formLabelWidth">
        ...
        <el-select clearable v-model="form.lineId" placeholder="请输入所属线路" filterable remote :remote-method="xianluFun">
          <el-option v-for="item in xianlus" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">杆 塔 号</label>
        <el-select clearable v-model="form.towerId" placeholder="请输入杆塔号" filterable remote :remote-method="TowerFun">
          <el-option v-for="item in towerNums" :key="item.towerId" :label="item.towerNum" :value="item.towerId"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="SIM卡号" :label-width="formLabelWidth">
        <el-select clearable v-model="form.simId" placeholder="请输入SIM卡号" filterable remote :remote-method="SIMFun">
          <el-option v-for="item in SIMs" :key="item.id" :label="item.num" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">安装方向</label>
        <el-select clearable v-model="form.deviceInstallationLocation" placeholder="请选择安装方向">
          <el-option v-for="item in deviceLocation" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="所属部门" :label-width="formLabelWidth">
        <el-select clearable v-model="form.departmentId" placeholder="请选择部门" filterable remote :remote-method="BuMenFun" @change="BanZuFun">
          <el-option v-for="item in departments" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">运维班组</label>
        <el-select clearable v-model="form.teamId" placeholder="请选择班组" filterable remote :remote-method="BanZuFun">
  ...
  <el-dialog title="修改设备信息" :visible.sync="xgbool" width="width">
    <el-form :model="upData">
      ...
      <el-form-item label="电压等级" :label-width="formLabelWidth">
        ...
        <label class="miniTitle">线路名称</label>
        <el-select clearable v-model="upData.lineId" :placeholder="lineIdTxt" filterable remote :remote-method="xianluFun">
          <el-option v-for="item in xianlus" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">杆 塔 号</label>
        <el-select clearable v-model="upData.towerId" :placeholder="towerIdTxt" filterable remote :remote-method="TowerFun">
          <el-option v-for="item in towerNums" :key="item.towerId" :label="item.towerNum" :value="item.towerId"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="SIM卡号" :label-width="formLabelWidth">
        <el-select clearable v-model="upData.simId" :placeholder="simIdTxt" filterable remote :remote-method="SIMFun" @change="test">
          <el-option v-for="item in SIMs" :key="item.id" :label="item.num" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">安装方向</label>
        <el-select clearable v-model="upData.deviceInstallationLocation" :placeholder="deviceInstallationLocationTxt">
          <el-option v-for="item in deviceLocation" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="所属部门" :label-width="formLabelWidth">
        <el-select clearable v-model="upData.departmentId" :placeholder="departmentIdTxt" filterable remote :remote-method="BuMenFun" @change="BanZuFun">
          <el-option v-for="item in departments" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
        <label class="miniTitle">运维班组</label>
        <el-select clearable v-model="upData.teamId" :placeholder="teamIdTxt" filterable remote :remote-method="BanZuFun">
          <el-option v-for="item in teams" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
  ...
  methods: {
    ...
    upDataFun() {
      ...
      if (this.upData.deviceInstallationLocation === "") {
        this.upData.deviceInstallationLocation = Number(this.rowDate.deviceInstallationLocation);
      }
    ...
    SIMFun(query) {
      if (query !== "") {
        ...
        Axios({
          method: "post",
          url: this.GLOBAL.AJAX_URL + "/v1/sim/get-unused-sims?key-world=" + query,
          headers: {
            Authorization: "Bearer " + Cookies.get("vue_admin_template_token")
          }
        }).then(msg => {
